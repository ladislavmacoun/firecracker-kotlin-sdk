...
public enum class CpuTemplate {
    ...
    @SerialName("T2") T2,
    @SerialName("T2S") T2S,
    @SerialName("T2CL") T2CL,
    @SerialName("T2A") T2A,
    @SerialName("V1N1") V1N1,
...
public data class MachineConfiguration(
    ...
    private fun validateConfiguration() {
        require(vcpuCount in 1..32) {
            "vCPU count must be between 1 and 32, got: $vcpuCount"
        }
        ...
        if (smt && vcpuCount > 1) {
            require(vcpuCount % 2 == 0) {
                "When SMT is enabled, vCPU count must be 1 or an even number, got: $vcpuCount"
            }
        }
        
        if (hugePages == HugePages.M2) {
            require(memSizeMib % 2 == 0) {
                "When using 2M huge pages, memory size must be a multiple of 2, got: $memSizeMib MiB"
            }
        }
    }
}